...
def test_live_rows_include_progress_from_pytest_percentage() -> None:
    ...
    assert row.progress_percent == 37


def test_lane_display_suppresses_lane_stream_output_by_default(capsys) -> None:
    reporter = LaneProgressReporter(clock=lambda: 100.0)
    reporter.register_lanes(["other"])
    display = build_lane_display(reporter)

    display.emit_lane_line("other", "hello from lane\n")

    assert capsys.readouterr().out == ""


def test_lane_display_falls_back_to_plain_when_rich_is_unavailable(
    monkeypatch,
) -> None:
    reporter = LaneProgressReporter(clock=lambda: 100.0)
    reporter.register_lanes(["other"])
    ...
    display = build_lane_display(reporter, show_lane_stream=True)

    assert isinstance(display, FakePlainDisplay)
    assert init_calls == [(reporter, True)]


def test_lane_display_prefers_rich_when_rich_is_available(
    monkeypatch,
) -> None:
    reporter = LaneProgressReporter(clock=lambda: 100.0)
    reporter.register_lanes(["other"])
    ...
    display = build_lane_display(reporter, show_lane_stream=False)

    assert isinstance(display, FakeRichDisplay)
