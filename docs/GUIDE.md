...
Everything beyond the quickstart in the
[README](https://github.com/HuzPro/pytest-lanes/blob/main/README.md).

- [Should you use this?](#should-you-use-this)
- [Measured results](#measured-results)
- [Defining lanes](#defining-lanes)
- [Command-line reference](#command-line-reference)
- [Configuration reference](#configuration-reference)
- [How it works](#how-it-works)
- [Coverage and JUnit reports in CI](#coverage-and-junit-reports-in-ci)
- [Prior art](#prior-art)
...
- **Every worker collects everything.** Each xdist worker imports and
  collects the full suite before running anything; lane subprocesses
  collect only their own paths.
...
you any. Two reproducible measurements ship in this repo, and both include
the cases where lanes lose.

### The bench example

[`examples/bench`](https://github.com/HuzPro/pytest-lanes/tree/main/examples/bench)
is a suite shaped like real integration suites: tests burn real CPU, heavy
lanes pay a per-process environment spin-up, and every module pays an
import cost. Measured on a 6-core CPU, medians of 3 rounds:
...
| **lanes-opt** (sharding + in-lane xdist) | **51.1s** | **2.48x** |

Plain lanes loses to xdist there because one heavy lane bounds the wall
time; the optimized config closes exactly that gap. Remove the import and
spin-up costs and xdist wins the same suite outright. Which model matches
your suite is an empirical question: heavy import graphs and expensive
per-process fixtures are the signal that lanes will pay. The example's
[README](https://github.com/HuzPro/pytest-lanes/blob/main/examples/bench/README.md)
decomposes the result.

### Validated on public suites

The same protocol (medians of 3 timed rounds after warm-up, identical test
populations and per-repo environment exclusions across every mode, a
6-core CPU) run against four well-known pytest suites, chosen to cover the
spectrum from container-heavy to pure-CPU. "best xdist" is the
best-performing safe `--dist` mode for that suite. "lanes tuned" is the
config `--lanes-suggest` generates from recorded durations, with
...
| litestar (`tests/unit`, docker-coupled tests excluded everywhere) | 59.1 | 33.4 | 54.0 | **20.2** | **lanes wins, but only the measured config.** The naive 5-lane split leaves one dominant lane (54.0); the duration-balanced partition beats their tuned loadgroup setup 1.65x. |
...
1. **Environment-level concurrency.** Each lane's setup-then-test sequence
   overlaps with the others'. This is the bulk of the win over serial
   pytest, and any parallel scheme gets some of it.
2. **Infrastructure paid once per environment, not once per worker.** The
   lane model starts one container per database lane per run; worker pools
   start one per worker that touches them. The cost of that duplication is
   usually memory and connection pressure rather than boot latency
   (containers boot concurrently), so do not expect this term to dominate
   on every suite.
3. **Scoped collection.** Every xdist worker collects the whole suite;
   each lane subprocess collects only its own paths.
4. **Ordering preserved inside a lane.** Not a speedup, a correctness
   property the speed depends on: no scheduling-dependent failures means
   no reruns.

The honest caveats: maximum parallelism equals your lane count; a single
slow lane bounds the wall-clock; the atomic unit is a **file**, so a suite
whose weight sits in one huge test file has a floor lanes cannot get under
(xdist's test-level distribution can, if your suite tolerates it); and on
a suite that is already fully concurrency-safe and well-balanced, a tuned
xdist lands within ~15%, sometimes ahead. The advantage concentrates where
suites actually live: partially concurrency-safe, with a few expensive
...
real Postgres testcontainers, lanes vs serial vs xdist, on your own
machine.
...
| `pytest --lane=postgres,redis` | Several lanes in-process. |
...
lane's record holds per-file test times plus measured `startup`
(collection end to first test, which captures container/fixture spin-up)
and `collect`; deleting that file resets scheduling to declared order.
...
Yes. CI-tested on Windows and Linux (Python 3.10-3.13). No `fork()`, no
